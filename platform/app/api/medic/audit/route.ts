...
6. FOR signal "honest_no_action_regression" / "honest_no_action_regression_fragment": the canned HONEST_NO_ACTION line was DELETED from sasa.ts on 2026-06-11 after 58 mis-fires in 7 days. If this pattern fires now, a regression has put the canned string back. Treat as P0 incident, set is_real_fumble=true, and the correction must (a) own the misread and stage the receipt/yes-variant the user actually sent, AND (b) flag the regression for engineering (the substitution should never reach a user under any path).
