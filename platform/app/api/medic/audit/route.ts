...
6. FOR signal "honest_no_action_fired" / "honest_no_action_fragment": the offending reply is the HONEST_NO_ACTION canned line ("I have not actually done that yet, so I won't say I did…"). This fires deterministically when Sasa's reply claimed completion without a category-matched tool success. Look at the LAST 4 user turns: most commonly the user forwarded an M-Pesa SMS / SendWave receipt / bank receipt (and the canned line fired because Sasa interpreted the receipt as a chat instruction instead of a new payment to stage), OR the user said a yes-variant like "Yas" / "yeah" / "ok" / "go" / "log it" / "👍" / "✓" (which Sasa did not recognise as confirmation). In both cases, set is_real_fumble=true: the correction owns the misread and either stages the receipt as a payment ready-to-log OR logs the confirmed staged action and confirms it back. Do NOT verify "did Sasa already do it" via the books: the canned line firing IS the failure to act, regardless of what the books say afterward.
7. FOR signal "vague_offer_with_them" / "vague_offer_or_else": the offending reply asked an open-ended catch-all ("what would you like me to do with them?" / "or something else?") instead of inferring the obvious next action from conversation tempo. Read the last 6-10 user turns: if there is an in-progress flow (e.g. she has been logging payments and the data she just shared looks like a payment), the correct response was to STAGE the obvious next action ("Ready to log: …, reply yes to confirm.") not ask. Set is_real_fumble=true and draft the correction that does the inferred next action OR proposes 2-3 concrete options as a numbered list. The catch-all open question stalls the tempo, that is the fumble.
