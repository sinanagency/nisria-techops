...
// Masked password with reveal-on-click + copy. The secret is NEVER in the page
// HTML — it's fetched from the server (decrypted) only when Nur clicks reveal.
export default function Reveal({ id }: { id: string }) {
  ...
  async function copy() {
    let s = secret;
    if (s == null) { const r = await revealSecret(id); if (r.secret == null) { setErr(r.error || "error"); return; } s = r.secret; setSecret(s); }
